...
def getbertemb(sent):
    ...
    with torch.no_grad():
        ...
        # print(f_emb_avg)
        # Sklearn only supports euclidean distance. Euclidean distance between
        # normalized vectors ammounts to cosine dist
        # https://stackoverflow.com/questions/38688319/why-doesnt-sklearns-distance-metric-class-have-cosine-distance
        return f_emb_avg/np.linalg.norm(f_emb_avg)

def get_nn_context(src):
    # only select unique sentences
    seen_sentences = set()
    context = []
    vectors = []
    for s in tqdm(src, desc="building index"):
        if s['sentence'] in seen_sentences:
            continue
        seen_sentences.add(s['sentence'])
        context.append(s)
        vectors.append(getbertemb(s['sentence']))
    nearest_neighbors = NearestNeighbors(n_neighbors = K+1,metric =\
                                         'euclidean').fit(vectors) 
    return context, nearest_neighbors

context, nearest_neighbors = get_nn_context(data_src)

for s in tqdm(data_tgt):
    ...
    if bert_data == bert_data_target:  # on training data, we are bound to find
        ...
        nns = nearest_neighbors.kneighbors([vector], return_distance=False)[0][1:]
        # sentence
    ...
    dataset.append({
        ...
        'context_sents': [context[x]['sentence'] for x in \
                          nns],
